...
import type { HookConfig, HookProviderKind } from "../agents/hooks-policy.ts";
...
function resolveHookProviderKind(
	providers: Record<string, { type: HookProviderKind }>,
): HookProviderKind {
...
async function installHooks(args: string[]): Promise<void> {
	...
	const sourceHooks = JSON.parse(sourceContent) as HookConfig;
	const providerKind = resolveHookProviderKind(config.providers);
	const normalizedSource = normalizeHookConfig(sourceHooks, {
	...
	targetConfig.hooks = normalizedSource.hooks;
